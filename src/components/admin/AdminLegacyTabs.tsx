...
export function AdminLegacyTabs({ tab, editSlug }: { tab: string; editSlug?: string }) {
  const searchParams = { edit: editSlug } as { edit?: string };
  const activeTab = tab;
  const isAdmin = true;
...
function Table({ headers, children }: { headers: string[]; children: React.ReactNode }) {
  return (
    <div className="visible-scrollbar w-full min-w-0 max-w-full overflow-x-auto">
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (isEditing) {
      ...
      setForm({
        ...
        label_host_bio: "", label_audience: "", label_partnership: "", label_eoi: "", label_videos: "", label_photos: "",
