...
function StrategistCard() {
  return (
    <div className="relative overflow-hidden rounded-2xl border border-border bg-card p-5">
      <div className="inline-flex items-center gap-1.5 rounded-full border border-border bg-background/50 px-2.5 py-1 text-[11px] font-medium uppercase tracking-wider text-lime">
        <Sparkles className="h-3 w-3" /> Strategist note
      </div>
      <h3 className="mt-4 font-display text-xl leading-snug">
        "Lean into UK garage-adjacent sounds this week — three tracks are compounding on TikTok and IG Reels is 48h behind."
      </h3>
      <p className="mt-3 text-sm text-muted-foreground">
        Best fit on your roster: indie-pop and dance-leaning artists. Format: sped-up snippet layered over a POV or backstage clip.
      </p>
      <div className="mt-5 flex items-center gap-2">
        <button className="inline-flex items-center gap-1 rounded-full bg-lime px-4 py-2 text-xs font-semibold text-primary-foreground hover:opacity-90">
          Build a brief <ArrowUpRight className="h-3.5 w-3.5" />
        </button>
        <button className="text-xs text-muted-foreground hover:text-foreground">See supporting trends</button>
      </div>
    </div>
  );
}
...
function RosterMatches() {
  const items = [
    { artist: "Nova Wren", tag: "Indie-pop · UK", match: "Sped-up snippet + backstage vlog", heat: 94 },
    { artist: "Kaine Voss", tag: "Hyperpop · US", match: "POV: first time hearing the drop", heat: 88 },
    { artist: "Marli & the Moon", tag: "Indie · AU", match: "Tour diary in one minute", heat: 76 },
  ];
  return (
    <div className="rounded-2xl border border-border bg-card p-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h3 className="mt-1 font-display text-lg">Matches on your artists</h3>
        </div>
        <Music2 className="h-4 w-4 text-muted-foreground" />
      </div>
      <ul className="mt-4 divide-y divide-border">
        {items.map((r) => (
          <li key={r.artist} className="flex items-center gap-3 py-3">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-secondary font-display text-sm">
              {r.artist.split(" ").map((w) => w[0]).slice(0, 2).join("")}
            </div>
            <div className="min-w-0 flex-1">
              <div className="truncate text-sm font-semibold">{r.artist}</div>
              <div className="truncate text-xs text-muted-foreground">{r.match}</div>
            </div>
            <div className="flex items-center gap-1 text-xs font-semibold text-lime">
              <TrendingUp className="h-3.5 w-3.5" /> {r.heat}
            </div>
          </li>
        ))}
      </ul>
